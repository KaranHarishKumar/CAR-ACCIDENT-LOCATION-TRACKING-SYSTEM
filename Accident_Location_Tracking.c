...
void setup()
{
  ...
  Serial.print(F("Start Application ")); 
...
void sendSMS(String message)
{
  ...
  delay(100);
 SIM900.println("AT + CMGS = \"+916383146519\"");  // recipient's mobile number, in international format
  delay(100);
...
int fahad2 = (longitude - datareal2 ) * 100000;
 ...
 textForSMS = textForSMS + " Message to Car";  
  sendSMS(textForSMS);
...
int fahad2 = (longitude - datareal2 ) * 100000;
  ...
  textForSMS = textForSMS + " Latitude: ";
   textForSMS.concat(datareal);
    textForSMS = textForSMS + ".";
  textForSMS.concat(fahad);
 textForSMS = textForSMS + "  limit switch activated";  
  sendSMS(textForSMS);
  Serial.println(textForSMS);
  Serial.println("message sent.");
  delay(5000);
